...
int replaceReg(FILE *fw)
{
	...
	{
		//需要获取栈中的一定区域
		printf("Wrong in get reg这时候出现了type = 1这种\n");
	...
	{
		printf("Wrong in get reg这时候出现了x=x+x这种，x在别的寄存器中\n");
		//fprintf(fw,"\t\tsw %s, %d($fp) \n", regName, val->offset);//溢出到内存
		val->type=2;
	}
	return index;
...
{
	/*if(op->kind == 6 || op->kind == 9)
	{
		printf("in SaveToStack %s\n", trans(op));
		return;
	}*/
...
char* get_reg(Operand op, FILE *fw)
{
	...
	else//变量第一次出现,补充：直接填入栈中
	{
		...
		{
			/*fprintf(fw, "the var is %s, and the key is  %d\n", op->u.val,regIndex);
			fprintf(fw, "reg = %d name =%s\n",Regs[regIndex].lastest_line_number, Regs[regIndex].varOperand->u.val);
			for(int i=0; i<REG_SIZE; i++)
			{
				fprintf(fw, "regindex = %d count =%d\n",i, Regs[i].lastest_line_number);
			}*/
			...
			{
				fprintf(fw, "\tmove %s, $fp\n", RegName(regIndex));
				fprintf(fw, "\taddi %s, %d\n", RegName(regIndex), tmpvar->offset);
			}
		}
		else if(op->kind == 9)//*a
		{
...
void forRead(FILE *fw, InterCode code)
{
	...
	fprintf(fw,"\taddi $sp, $sp, 4\n");
	//补充
	if(code.u.single.op->kind == 6 || code.u.single.op->kind == 9)
	{
		fprintf(fw,"\tsw $v0, 0(%s)\n", pointer_get_reg(code.u.single.op, fw));//把v0更新到原来的寄存器中
		printf("we have read *a in forread\n");
	}
	else
	{
...
void forWrite(FILE *fw, InterCode code)
{
	...
	{
		if(item->type == 1)
			printf("Exception1 in forwrite\n");
		else if(item->type == 2)
		{
			printf("we find \n");
			Regs[item->reg_no].used = false;
		}
		item->type = 2;
	}
}
...
void forAssign(FILE *fw, InterCode code)
{
	...
	else if(code.u.assign.left->kind == 9 || code.u.assign.left->kind == 6)//*a = ...
	{
		...
		fprintf(fw, "\tsw %s, 0(%s)\n", get_reg(code.u.assign.right, fw), pointer_get_reg(code.u.assign.left, fw));
		SaveToStack(code.u.assign.right, fw);
	}
	else
	{
		//printf("%s := %s\n",trans(code.u.assign.left), trans(code.u.assign.right));
		//printf("left type = %d, right type = %d", code.u.assign.left->kind, code.u.assign.right->kind);
		fprintf(fw, "\tmove %s, %s\n", get_reg(code.u.assign.left, fw), get_reg(code.u.assign.right, fw));
		SaveToStack(code.u.assign.left, fw);
		SaveToStack(code.u.assign.right, fw);
	}
...
void forCall(FILE *fw, InterCode code)
{
	...
	for(int i=0; i<=arg_Count-4; i++)
	{
		...
		else
		{
			...
			{
				fprintf(fw,"\tsw %s, 0($sp) \n", RegName(arg->reg_no));
				printf("Unexpect in forcallfunction\n");
			}
			else if(arg->type==2)
			{
...
void forAdd(FILE *fw, InterCode code)
{
	...
	if(code.u.binop.op1->kind == CONST)
	{
		...
		else
		{
			...
			SaveToStack(code.u.binop.result,fw);SaveToStack(code.u.binop.op2,fw);
			return;
		}
	}
	else if(code.u.binop.op2->kind == CONST)
	{
		...
		else
		{
			...
			SaveToStack(code.u.binop.result,fw);SaveToStack(code.u.binop.op1,fw);
			return;
		}
	}	
	else if(code.u.binop.result->kind == 9 || code.u.binop.result->kind == 6)//*a = ...
	{
		...
		SaveToStack(code.u.binop.op1,fw);SaveToStack(code.u.binop.op2, fw);
		return;
	}
	else
	{
		fprintf(fw,"\tadd %s, %s, %s\n", get_reg(code.u.binop.result,fw), get_reg(code.u.binop.op1,fw), get_reg(code.u.binop.op2, fw));
		SaveToStack(code.u.binop.result,fw);SaveToStack(code.u.binop.op1,fw);SaveToStack(code.u.binop.op2, fw);
		return;
	}
}
...
bool testReg()
{
	for(int i=0; i<REG_SIZE; i++)
	{
		if(Regs[i].used==true)
		{
			Operand op = Regs[i].varOperand;
			char* regName = RegName(i);
			char *keychar=trans(Regs[i].varOperand);
			VarDescriptor* val = getItemFromVarDescriptor(keychar);
			if(val==NULL) ;//补充可能遇到这类情况
			else if(val->type==1)
			{
				printf("Wrong!\n");
				return false;
